...
export class LazyRoleIsolateTestComponent implements OnInit {
  ...
  ngOnInit() {
    this.configService.addPermissionStrategy('lol', (tf: any) => {
      this.renderer.setAttribute(tf.elementRef.nativeElement.nextSibling, 'ng-hide', 'true');
    });

    this.configService.setDefaultOnUnauthorizedStrategy('lol');
    this.permissionsService.addPermission('ADMIN123');
    this.rolesService.addRole('ADMIN123', ['NICE']);
  }
